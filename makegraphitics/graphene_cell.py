# ----------------------------------------------------------------------------
#
# Revision 2022-04-01: Removed the need to call this with a specified
#                      force field. The forcefield parameters were used
#                      to get the cc_bond_length and the layer gap.
#                      They are now default set to the equilbrium (T=0) values
#                      for graphene/graphite as
...
class Graphene(Molecule):
    ...
    def __init__(self, cc_bond_length=1.4148, layer_gap=3.4827):
        self.CC = cc_bond_length
    ...
    def assign_atom_states(self, lattice_dimensions):
        ...
        for x in range(lattice_dimensions[0]):
            for y in range(lattice_dimensions[1]):
                for z in range(lattice_dimensions[2]):
                    atom_states.extend(list(cell_charges))
        return atom_states
    ...
    # ------------------------------------------------------------------------
    #
    # add_cross_bond:  Fixes the bonds connecting cells.
    #
    # ------------------------------------------------------------------------
    def add_cross_bond(self, lattice_dimensions, cell_position,
                       atoms, bonds, i):
    ...
    # ------------------------------------------------------------------------
    #
    # For pristine graphene with PBC we only have a single
    # bond_type, angle_type, dihedral_type and improper_type
    #
    # ------------------------------------------------------------------------
