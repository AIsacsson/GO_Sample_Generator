...
class Connector(object):
    ...
    #Create a list which tells which type of bond each bond is
    #It looks lie we are doing the same thing as in find_bond_types.
    #They could be the same routing, but compartmentalisation is more 
    #clean I guess. Otherwise, straight forward routine.
    def bond_labels(self, atom_labels, bonds, bond_types):
        bond_labels = []
     
        for bond in bonds:
            atoms = [atom_labels[int(bond[0]) - 1], 
                     atom_labels[int(bond[1]) - 1]]
    ...
    #I guess the reason for the weird labeling of angles comes from the 
    #fact that we use the bond graph. The way the bond graph is generated, the
    #bonds in it are not in any special order.
    def angles(self, bonds, bond_graph):
        ...
        for centre in range(1, N + 1):
            neighbours = list(bond_graph[centre - 1]) #Here we pay for not adding
            neighbours = [x + 1 for x in neighbours]  #the +1 in the bond graph
    ...
    # ------------------------------------------------------------------------
    #
    # find_bond_types: This is to figure out how many different types of bonds
    #                  we have. Basically we go through the bonds-array one by
    #                  one. Then it checks the atom types at the two points and
    #                  add to the type list if they are not already there.
    #
    # ------------------------------------------------------------------------
    def find_bond_types(self, atom_labels, bonds):
        bond_types = []

        for bond in bonds:  # Start looping throgh the bond list

            atoms = [atom_labels[int(bond[0]) - 1],  # The two atom types
                     atom_labels[int(bond[1]) - 1]]  # conected by the bonds
            found = False
            for i in range(len(bond_types)):    # Loop through those we know
                if bond_types[i] == atoms:      # already and check if we
                    found = True                # already have it
                    break
                if bond_types[i] == list(reversed(atoms)):  # as [n,m]=[m,n]
                    found = True                            # we have to check
                    break                                   # both ways
            if not found:                   # We found a new bond type to add
                bond_types += [atoms]       # to the list.
        return bond_types

    # ------------------------------------------------------------------------
    #
    # find_angle_types: This is the equivalent to figure out the bond types.
    #                   Basically we go through the bonds-array one by
    #                   one. Then it checks the atom types at the two points
    #
    # ------------------------------------------------------------------------
